...
# =======================================================================================================
    ...
    st.markdown("💡 **月报模式**：上传当月与去年同月的数据，自动生成 2x2 四宫格排行榜。")

    col1, col2, col3 = st.columns(3)
    with col1: file_curr = st.file_uploader("1. 📥 【今年当月】生意参谋数据", type=["xlsx", "xls", "csv"], key="m1")
    with col2: file_last = st.file_uploader("2. 📥 【去年当月】生意参谋数据", type=["xlsx", "xls", "csv"], key="m2")
    with col3: file_map = st.file_uploader("3. 🏷️ 【分类映射表】", type=["xlsx", "xls", "csv"], key="m3")

    def load_data(file):
        if file.name.endswith('.csv'):
            try: df = pd.read_csv(file, encoding='gbk')
            except:
                file.seek(0)
                    ...
                    for row_idx, row in enumerate(df.values):
                        for col_idx, val in enumerate(row):
                            ...
                            if isinstance(val, (int, float)):
                                if 'Share%' in col_name or 'YOY' in col_name or '率' in col_name: fmt = pct_fmt
                                elif 'Value' in col_name or 'QTY' in col_name or '人数' in col_name: fmt = num_fmt
                            if pd.isna(val): worksheet.write(start_row + 1 + row_idx, start_col + col_idx, "-", cell_fmt)
                            else: worksheet.write(start_row + 1 + row_idx, start_col + col_idx, val, fmt)
    ...
    if st.button("⚡ 生成月度排行 (HAY Ranking)", type="primary", use_container_width=True):
        if file_curr and file_last and file_map:
            with st.spinner('正在进行精密计算与排版...'):
                ...
                for col in numeric_columns:
                    if col in df_merged.columns: df_merged[col] = to_numeric_col(df_merged[col])
                ...
                df_merged['YOY'] = np.where(df_merged['去年支付金额'] > 0, (df_merged['Value'] - df_merged['去年支付金额']) / df_merged['去年支付金额'], np.nan)
            ...
            st.warning("⚠️ 请在左侧依次上传：1.今年当月数据 2.去年当月数据 3.分类映射表")
